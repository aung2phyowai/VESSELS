...
{
    class AndroidAppCore : DrawableGameComponent2
    {
        #region Fields
        ...
        //Debug Phone IP address 10.252.28.213 // camera uri
        String camUri = "http://10.252.28.213:1234/videofeed";  //Android debug phone
        ...
        //Constructor
        public AndroidAppCore(ScreenManager screenManager)
        {
        ...
        public override void Initialize()
        {
            ...
            send_to_address = IPAddress.Parse("10.252.28.213");          //Android ip (debug phone)
        ...
        public override void Draw(TimeSpan elapsedTime, TimeSpan totalTime)
        {
            ...
            try
            {
                ...
                if (camImage != null)
                {
                    spriteBatch.Draw(camImage, new Vector2(SCREENWIDTH / 2.0f - ((camImage.Width * videoScale) / 2.0f), SCREENHEIGHT / 2.0f - ((camImage.Height * videoScale) / 2.0f)), null, Color.White, 0, Vector2.Zero, videoScale, SpriteEffects.None, 0);
                }
                else
                {
        ...
        private void InitializePointsForArrow(){
            ...
            //Radial
            cnt = 0; float[] radius = { r2 + 60, r2 - 55 };  //middle ring(ring closest to stimuli): r2+60. inner ring (ring furthest from stimuli): r2-85
            for (int j = 0; j < 2; j++)
            {
                for (int i = 1; i <= 8; i++)
                {
