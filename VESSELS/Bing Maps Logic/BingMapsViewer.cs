...
namespace VESSELS.BingsMapLogic
{
    ...
    class BingMapsViewer
    {
            ...
            GeoCoordinate location, int gridSize, int zoomlevel, SpriteBatch spriteBatch)
        {
            ...
            //Set Tile Dimensions. Note: tile dimensions cannot be greater than 1400 x 900
            //tileDimensions = new Vector2(screenBounds.Width, screenBounds.Height);
            float aspectRatio = (float)screenBounds.Width / (float)screenBounds.Height;
            
            //tileDimensions = new Vector2(900,(float)Math.Round((float)900/aspectRatio));
            //tileDimensions = new Vector2(1400, (float)1400 / aspectRatio);
            tileDimensions = new Vector2(600, 400);
           ...
           // maxOffsetAbs = tileDimensions * ((ActiveTiles.ActiveTilePlaneSize - 1) / 2);     
            maxOffsetAbs.X = ((tileDimensions.X * ActiveTiles.ActiveTilePlaneSize) / 2) - screenBounds.Width / 2;
            maxOffsetAbs.Y = ((tileDimensions.Y * ActiveTiles.ActiveTilePlaneSize) / 2) - screenBounds.Height / 2;
            ...
            for (int i = 0; i < ActiveTiles.ActiveTilePlaneSize; i++)
            {
                for (int j = 0; j < ActiveTiles.ActiveTilePlaneSize; j++)
                {
                   ...
                   // Texture2D image = ActiveTiles[i, j].Image;
                    if (ActiveTiles[i, j].Image == null)
                    {
                        SpriteBatch.Draw(defaultImage, screenCenterVector + Offset + extraOffset, null, Color.White, 0,
                            tileDimensions / 2f, 1, SpriteEffects.None, 0);
                    }
                    else
                    {
                        SpriteBatch.Draw(ActiveTiles[i, j].Image, screenCenterVector + Offset + extraOffset, null, Color.White, 0,
                            tileDimensions / 2f, 1, SpriteEffects.None, 0);
                    }
                }
            }
        ...
        public void MoveByOffset(Vector2 gestureOffset)
        {
            ...
            if (Offset.X < -maxOffsetAbs.X + 800 || Offset.X > maxOffsetAbs.X)                
            {
                fixOffset.X = -gestureOffset.X;
            }
            if (Offset.Y < -maxOffsetAbs.Y +480 || Offset.Y > maxOffsetAbs.Y)
            {
